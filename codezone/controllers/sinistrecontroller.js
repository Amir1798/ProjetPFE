...
const HouseInsurance = require('../models/carinsurancemoidel');
...
const addSinistre = async (req, res, next) => {
    try {
        ...
        if (!insurance) {
            const error = appError.create('No insurance found for this user', 404, statusHttp.ERROR);
            return next(error);
        }
...
const getUserSinistres = async (req, res, next) => {
    try {
        ...
        if (userSinistres.length === 0) {
            const error = appError.create('No sinistres found for this user', 404, statusHttp.ERROR);
            return next(error);
        }
